...
liability patterns sooner. The latter is a monotone submodular-coverage idea
...
finding a liability. We therefore ask whether profile diversity can be used
...
clinical kinase-drug parent structures and 146 commonly measured kinases
\citep{klaeger2017,zdrazil2024}. A recorded binding liability is an exact
$K_d$ within tenfold of the compound's best measured documented-target $K_d$;
documented targets are excluded from candidates. \textbf{PKIS2} contributes
640 parent structures and 392 parent kinases from 1~$\mu$M percent-inhibition
profiles \citep{drewry2017}; activity is strictly $>90\%$, with $>80\%$
...
constructs sharing a parent target are max-collapsed, salts are reduced to the
...
thresholds are operational labels, not potency estimates. Max-collapsing assay
constructs changes the biological question, although the construct-level audit
did not reverse the external result. Morgan similarity and reference profiles
...
A kinase-assay expert must review four points before submission or reuse:
(i) parent-target collapse across mutants and complexes; (ii) comparability of
Nanosyn percent inhibition, PKIS2 inhibition, and Kinobeads $K_d$ while keeping
their results separate; (iii) whether $>80/>90$ and relative-$K_d$ labels are
appropriate for the stated counter-screen decision; and (iv) representative
orders for known assay artifacts and promiscuous compounds. No such review is
fabricated here.
...
The release includes a checklist with source-record examples for expert review.
Any paper revision following expert feedback must distinguish corrections of
biological interpretation from outcome-driven changes to the algorithm. A
